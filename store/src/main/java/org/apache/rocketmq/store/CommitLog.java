...
public class CommitLog {
     ...
     * 主要是恢复MapedFileQueue对象的commitedWhere变量值（即刷盘的位置）
     */
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            ...
            // global
            msg.setStoreTimestamp(beginLockTimestamp);
            //消息存储：5、判断MappedFile是否为空或者满了则新创建一个MappedFile
            if (null == mappedFile || mappedFile.isFull()) {
                mappedFile = this.mappedFileQueue.getLastMappedFile(0); // Mark: NewFile may be cause noise
            }
            if (null == mappedFile) {
            ...
            //消息存储：6、将消息内容写入MappedFile.mappedByteBuffer：MappedByteBuffer对象，即写入消息缓存中；由后台服务线程定时的将缓存中的消息刷盘到物理文件中；
            result = mappedFile.appendMessage(msg, this.appendMessageCallback);
            switch (result.getStatus()) {
                case PUT_OK:
                    break;
    ...
    public PutMessageResult putMessages(final MessageExtBatch messageExtBatch) {
        ...
        try {
            ...
            // global
            messageExtBatch.setStoreTimestamp(beginLockTimestamp);
            //消息存储：6、mappedFile为空或者已满，则创建mappedFIle
            if (null == mappedFile || mappedFile.isFull()) {
                mappedFile = this.mappedFileQueue.getLastMappedFile(0); // Mark: NewFile may be cause noise
            }
            if (null == mappedFile) {
            ...
            switch (result.getStatus()) {
                case PUT_OK:
                    break;
                case END_OF_FILE: //存储byteBuffer空间小于消息大小，则创建一个新的MappedFile，再次执行mappedFile.appendMessages（L090）；其余则原样抛回异常
                    unlockMappedFile = mappedFile;
                    // Create a new file, re-write the message
                    mappedFile = this.mappedFileQueue.getLastMappedFile(0);
                    if (null == mappedFile) {
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            //消息存储：10、消息追加--写之前判断消息的文件长度是否可以写入
